...
class ContentType(Enum):
    ...
    @classmethod
    def sort(cls, file: str | Path | None = None) -> "ContentType":
        """
...
class SupportedDataTypes:
    ...
    @classmethod
    def check_valid_data(cls, file_names: Sequence[str | Path]) -> bool:
        """
        ...
        if file_type == ContentType.tag:
            ...
            if path.name == "__star__.tag":
                ...
                if trash_path.is_file():
                    ...
                    error_msg.setWindowTitle("Deleting __star__.tag")
                    error_msg.exec_()
                    return
                else:
                    self.star = True
                ...
                if star_path.is_file():
                    ...
                    error_msg.setWindowTitle("Deleting __trash__.tag")
                    error_msg.exec_()
                    return
                else:
                    self.trash = True
                ...
                if interrupted_path.is_file():
                    ...
                    error_msg.setWindowTitle("Deleting __complete__.tag")
                    error_msg.exec_()
                    return
                else:
                    self.complete = True
                ...
                if complete_path.is_file():
                    ...
                    error_msg.setWindowTitle("Deleting __interrupted__.tag")
                    error_msg.exec_()
                    return
                else:
                    self.interrupted = True
...
class FileModel(QtGui.QStandardItemModel):
    ...
    def __init__(
        ...
        if watcher_on:
            ...
            self.watcher_thread: QtCore.QThread | None = QtCore.QThread(parent=self)
    ...
    @Slot()
    def on_renaming_file(self, item: Item | None = None) -> None:
        """
    ...
    def sort_and_add_item(
        self, folder_path: Path, files_dict: dict | None = None
    ) -> bool | None:
        """
        ...
        if folder_path == self.monitor_path:
            if files_dict is not None:
                ...
                LOGGER.warning(
                    "Files have been found in the monitoring folder, please remove them"
                )
            return False
...
class FileTreeView(QtWidgets.QTreeView):
    ...
    def __init__(self, proxy_model: SortFilterProxyModel, parent: Any | None = None):
        """
    ...
    @Slot(object)
    def on_adjust_column_width(self, item: Item | None = None) -> None:
        """
    ...
    def create_collapsed_state(self, incoming_item: Item | None = None) -> None:
        """
...
class FilterWorker(QtCore.QObject):
    ...
    def filter_items(
        ...
        filter: str,
        tag_filter: list[str] = [],
    ) -> tuple[dict[Path, Item], dict[str, list[str]]] | None:
        """
        ...
        trashed_dict: dict[Path, Item] | None = {}
        ...
        parent_dict: dict[Path, Item] | None = {}
        child_dict: dict[Path, Item] | None = {}
        for path, item in current_dict.items():
            if self.thread().isInterruptionRequested():
                return None
            parent = item.parent()
    ...
    def _add_parent(
        self,
        item: Item,
        adding_dict: dict[Path, Item] | None,
    ...
    def _add_children(
        ...
        item: Item,
        adding_dict: dict[Path, Item] | None,
        trashed_dictionary: dict[Path, Item] | None,
    ) -> dict[Path, Item] | None:
        """
    ...
    ) -> tuple[dict[Path, Item] | None, dict[Path, Item]]:
        """
...
class FileExplorer(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.loading_label: IconLabel | None = None
        ...
        self.filter_thread: QtCore.QThread | None = None
    ...
    def on_existing_item_files_updated(self, item: Item) -> None:
        ...
        if should_item_show:
            if item not in self.proxy_model.allowed_items:
                self.proxy_model.allowed_items.append(item)
    ...
    def on_create_path_list(self, item_index: QtCore.QModelIndex | None = None) -> None:
        """
        ...
        for index, data in enumerate(self.data):
            ...
            for name, value in data.data_items():
                ...
                if name in data.dependents():
                    ...
                    column_content.append("Independent")
...
class TextEditWidget(TextViewWidget):
    ...
    def save_activated(self) -> None:
        ...
        try:
            ...
            error_msg.setWindowTitle("Error trying to save markdown edit.")
            error_msg.exec_()
        ...
        if response:
            ...
            try:
                ...
                if not comment_path.is_file():
                    ...
                    error_msg.setWindowTitle("Error trying to save comment.")
                    error_msg.exec_()
...
class ImageViewer(QtWidgets.QLabel):
    ...
    def __init__(self, path_file: Path, *args: Any, **kwargs: Any):
        ...
        # except Exception as e:
            ...
            self.setText("Image could not be displayed")
            LOGGER.error(e)
...
class IconLabel(QtWidgets.QLabel):
    def __init__(
        self, movie: QtGui.QMovie, size: int | None = None, *args: Any, **kwargs: Any
    ):
        """
...
class Monitr(QtWidgets.QMainWindow):
    def __init__(
        self, monitorPath: str = ".", parent: QtWidgets.QMainWindow | None = None
    ):
        super().__init__(parent=parent)
        ...
        self.data_window: Collapsible | None = None
        self.text_input: Collapsible | None = None
        self.file_windows: list[Collapsible] = []
        self.scroll_area: VerticalScrollArea | None = None
        self.tags_label: TagLabel | None = None
        self.tags_creator: TagCreator | None = None
        self.invalid_data_label: QtWidgets.QLabel | None = None
        self.header_label: QtWidgets.QLabel | None = None
        self.loading_label: IconLabel | None = None
        self.loading_movie = QtGui.QMovie(
            os.path.join(plottrPath, "resource", "gfx", "loading_gif.gif")
        )
        ...
        self.data_file_need_update: Path | None = None
        ...
        # Threading stuff
        self.loader_worker: LoaderWorker | None = None
        self.loader_thread: QtCore.QThread | None = None
    ...
    def print_model_main_dictionary(self) -> None:
        ...
        print("Here comes the model main dictionary")
    ...
    def add_data_window(self, data_files: dict) -> None:
        ...
        if len(data_files["data"]) < 1:
            self.invalid_data_label = QtWidgets.QLabel("No data to display.")
