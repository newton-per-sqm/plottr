...
def largest_numtype(arr: np.ndarray, include_integers: bool = True) -> None | type:
    """
...
def arrays_equal(a: np.ndarray, b: np.ndarray, rtol: float | None = None) -> bool:
    ...
    * invalid entries (capturing both ``None`` and ``np.nan``; all treated as
    ...
    close: np.ndarray | np.bool_ = np.zeros_like(a, dtype=bool)
...
def array1d_to_meshgrid(
    arr: list | np.ndarray, target_shape: tuple[int, ...], copy: bool = True
) -> np.ndarray:
    """
...
def find_direction_period(vals: np.ndarray, ignore_last: bool = False) -> float | None:
    """
