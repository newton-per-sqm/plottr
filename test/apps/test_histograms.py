...
def testdata(n_reps=100, n_extra_axes=1):
    ...
    extra_axes_vals = [np.linspace(-1.0, 1.0, 10 + i) for i in range(n_extra_axes)]
    ...
    dataset = DataDict(
        result=dict(values=outcomes_mesh.flatten(), axes=axes),
    )
...
def complex_testdata(n_samples=100, n_amps=21):
    ...
    dataset = DataDict(
        ...
        result=dict(values=vv.flatten(), axes=["sample", "amp"]),
    )
...
def complex_testdata_many_independents(n_samples=100, n_amps=4, n_phases=8, n_widths=3):
    samples = np.arange(n_samples)
    amps = np.arange(n_amps) + 1.0
    phases = np.linspace(0, 2 * np.pi * (1.0 - 1.0 / n_phases), n_phases)
    widths = (np.arange(n_widths) + 1.0) / 5.0
    ...
    dataset = DataDict(
        ...
        result=dict(values=vv.flatten(), axes=["sample", "amp", "phase", "width"]),
    )
...
def plot() -> tuple[Flowchart, AutoPlotMainWindow]:
    ...
    win = AutoPlotMainWindow(
        fc, loaderName=None, widgetOptions=widgetOptions, monitor=False
    )
    win.show()
    return fc, win
