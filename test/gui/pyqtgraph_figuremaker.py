...
def test_basic_line_plot():
    ...
    with FigureMaker() as fm:
        line_1 = fm.addData(
            x, y, labels=["x", "cos(x)"], plotDataType=PlotDataType.line1d
        )
        _ = fm.addData(
            x,
            y**2,
        ...
        line_2 = fm.addData(
            x, np.abs(y), labels=["x", "|cos(x)|"], plotDataType=PlotDataType.line1d
        )
    return fm.widget
...
def test_images():
    ...
    with FigureMaker() as fm:
        img1 = fm.addData(
            xx, yy, zz, labels=["x", "y", "fake data"], plotDataType=PlotDataType.grid2d
        )
        img2 = fm.addData(
            xx,
            yy,
            zz[:, ::-1],
            labels=["x", "y", "fake data (mirror)"],
