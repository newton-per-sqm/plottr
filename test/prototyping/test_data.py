...
def oscillating_test_data(*specs: tuple[float, float, int], amp=1, of=0):
    ...
    data = (
        ...
        * np.prod(
            np.array(
                [np.cos(2 * np.pi * (f * x + p)) for x, (f, p, _) in zip(axes, specs)]
            ),
            axis=0,
        )
        + np.random.normal(loc=0, scale=1, size=(axes[0].shape))
        + of
    )
    dd = MeshgridDataDict()
    for i, a in enumerate(axes):
        dd[f"axis_{i}"] = dict(values=a)
    dd["data"] = dict(axes=[f"axis_{i}" for i in range(len(specs))], values=data)
    dd.validate()
    return dd
