...
def test_data_extraction(qtbot):
    ...
    assert np.all(
        np.isclose(data.data_vals(data_name), out.data_vals(data_name), atol=1e-15)
    )
...
def test_data_extraction2(qtbot):
    ...
    assert np.all(
        np.isclose(
            data.data_vals(data_names[0]), out.data_vals(data_names[0]), atol=1e-15
        )
    )

    assert np.all(
        np.isclose(
            data.data_vals(data_names[1]), out.data_vals(data_names[1]), atol=1e-15
        )
    )
