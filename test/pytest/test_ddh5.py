...
def test_basic_storage_and_retrieval():
    ...
    data = dd.DataDict(
        x=dict(
            values=x, unit="A", __info__="careful!", __moreinfo__="more words in here"
        ),
...
def test_appending():
    ...
    data.add_data(
        x=[4], y=np.linspace(0, 1, 5).reshape(1, -1), z=np.arange(5).reshape(1, -1)
    )
    assert data.validate()
...
def test_loader_node(qtbot):
    ...
    data.add_data(
        x=[3], y=np.linspace(0, 1, 5).reshape(1, -1), z=np.arange(5).reshape(1, -1)
    )
    dds.datadict_to_hdf5(data, str(FILEPATH), append_mode=dds.AppendMode.new)
    assert _clean_from_file(dds.datadict_from_hdf5(str(FILEPATH))) == data
...
def _mkdatachunk(start, nrows, npts=1):
    numbers = np.arange(start, start + nrows).reshape(nrows, -1) * np.ones(
        npts
    )  # broadcasts to (nrows, npts)
    return numbers
...
class _Writer(Process):
    ...
    def run(self):
        data = self.mkdata()
        with dds.DDH5Writer(
            dd.str2dd("x[W]; y[T](x)"), filepath=self.filepath
        ) as writer:
