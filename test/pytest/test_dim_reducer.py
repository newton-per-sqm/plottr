...
def test_reduction(qtbot):
    ...
    assert num.arrays_equal(fc.outputValues()["dataOut"].data_vals("vals"), vals)
...
def test_xy_selector(qtbot):
    ...
    assert num.arrays_equal(
        fc.outputValues()["dataOut"].data_vals("vals"), vals[:, :, 0]
    )

    # try a different reduction on the third axis
    node.reductions = {"z": (ReductionMethod.average, [], {})}
    assert num.arrays_equal(
        fc.outputValues()["dataOut"].data_vals("vals"), vals.mean(axis=-1)
    )
