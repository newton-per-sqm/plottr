...
from plottr.apps.monitr import FileModel, FilterWorker
from plottr.data.datadict import DataDict
from plottr.data.datadict_storage import datadict_to_hdf5

FOLDER_PATH = Path(
    r"C:\Users\Msmt\Documents\Python Scripts\watchdog_testing_data\automatic_file_creation"
)
...
def generate_data(file_size, file_path=None, ret=False) -> DataDict:
    ...
    if file_path is None:
        datadict_to_hdf5(
            datadict=datadict,
            path=os.path.join(os.path.abspath(os.getcwd()), "data.ddh5"),
        )
        return datadict
    else:
...
def generate_file_structure(
    folder_path=None,
) -> tuple[list[Path], list[Path], FileModel]:
...
def test_only_ddh5_files(tmp_path, qtbot):
    ...
    queries = []
    allowed_list, queries_dict = filter_worker.filter_items(
        model, False, False, "", queries
    )
...
def test_same_tag_in_2_datasets(tmp_path, qtbot):
    ...
    filtered_list, queries_dict = filter_worker.filter_items(
        model, star_status, trash_status, queries, tag_queries
    )
    ...
    for path, item in model.main_dictionary.items():
        if path in allowed_folders:
            assert FilterWorker.is_item_shown(
                item, queries, tag_queries, star_status, trash_status
            )
        else:
            assert not FilterWorker.is_item_shown(
                item, queries, tag_queries, star_status, trash_status
            )
...
def test_2_different_children_tags(tmp_path, qtbot):
    ...
    tag_queries = []
    star_status, trash_status = False, False
    invalid_allowed_list, queries_dict = filter_worker.filter_items(
        model, star_status, trash_status, queries, tag_queries
    )
    ...
    queries = "t:favorite"
    favorite_allowed_list, queries_dict = filter_worker.filter_items(
        model, star_status, trash_status, queries, tag_queries
    )
...
def test_2_different_parent_tags(tmp_path, qtbot):
    ...
    favorite_results = (
        [days_paths[0], days_paths[2]] + folder_paths[:3] + folder_paths[6:9]
    )
    invalid_results = (
        [days_paths[1], days_paths[2]] + folder_paths[3:6] + folder_paths[6:9]
    )
...
def test_combination_of_star_trash_tags(qtbot, tmp_path):
    ...
    for path in coffee_tag:
        with open(path.joinpath("coffee.tag"), "w") as f:
            f.write("this is a cofee")

    days_paths_copy = days_paths.copy()
    days_paths_copy.pop(1)

    trash_hidden_allowed = (
        days_paths_copy
        + [unique_named_path, lonely_child]
        + folder_paths[0:3]
        + folder_paths[7:14]
    )
    ...
    tag_queries = []
    filtered_list, queries_dict = filter_worker.filter_items(
        model, False, True, queries, tag_queries
    )
    paths = [item for item in filtered_list]
    ...
    tag_queries = []
    filtered_list, queries_dict = filter_worker.filter_items(
        model, False, False, queries, tag_queries
    )
    paths = [item for item in filtered_list]
    ...
    tag_queries = [""]
    filtered_list, queries_dict = filter_worker.filter_items(
        model, True, False, queries, tag_queries
    )
    paths = [item for item in filtered_list]
    assert sorted(star_only_allowed) == sorted(paths)

    coffee_and_star_allowed = [
        days_paths[0],
        unique_named_path,
        lonely_child,
        days_paths[4],
    ...
    tag_queries = [""]
    filtered_list, queries_dict = filter_worker.filter_items(
        model, True, True, queries, tag_queries
    )
    paths = [item for item in filtered_list]
    assert sorted(star_and_trash_allowed) == sorted(paths)

    star_trash_coffee_allowed = [
        days_paths[0],
        days_paths[4],
        lonely_child,
        unique_named_path,
    ...
    star_trash_coffee_favorite_allowed = [
        days_paths[0],
        unique_named_path,
        lonely_child,
    ]
...
def test_item_query_check(qtbot, tmp_path):
    ...
    assert FilterWorker._item_check(
        item,
        False,
        queries_dict,
    )
    ...
    tag_filter = []
    queries_dict = FilterWorker.parse_queries(filter, tag_filter)
    assert not FilterWorker._item_check(
        item,
        False,
        queries_dict,
    )
