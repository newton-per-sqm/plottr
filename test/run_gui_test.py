    ...
    for mod in mods:
        for name, fun in inspect.getmembers(mod):
            if inspect.isfunction(fun) and "test_" in name:
                functions[f"{mod.__name__}.{name}"] = dict(
                    func=fun, signature=inspect.signature(fun)
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Testing data display widgets",
    ...
    parser.add_argument(
        "--kwargs", help="keyword arguments for the function", default={}
    )
